...
export default ((userOpts?: Partial<ExplorerOptions>) => {
  ...
  const ExplorerComponent: QuartzComponent = (props: QuartzComponentProps) => {
    ...
    const sortLabels = isChinese
      ? {
          label: "排序笔记",
          nameAsc: "文件名 (A-Z)",
          nameDesc: "文件名 (Z-A)",
          modifiedDesc: "编辑时间（从新到旧）",
          modifiedAsc: "编辑时间（从旧到新）",
    ...
    return (
      ...
      >
        ...
        <div class="explorer-header">
          ...
          <details class="explorer-sort">
            <summary aria-label={sortLabels.label} title={sortLabels.label}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="18"
                height="18"
            ...
            <div class="explorer-sort-menu" role="menu" aria-label={sortLabels.label}>
              <button type="button" role="menuitemradio" data-sort="name-asc">
                {sortLabels.nameAsc}
              </button>
              <button type="button" role="menuitemradio" data-sort="name-desc">
                {sortLabels.nameDesc}
              </button>
              <hr />
              <button type="button" role="menuitemradio" data-sort="modified-desc">
                {sortLabels.modifiedDesc}
              </button>
              <button type="button" role="menuitemradio" data-sort="modified-asc">
                {sortLabels.modifiedAsc}
              </button>
              <hr />
              <button type="button" role="menuitemradio" data-sort="created-desc">
                {sortLabels.createdDesc}
              </button>
              <button type="button" role="menuitemradio" data-sort="created-asc">
                {sortLabels.createdAsc}
              </button>
            </div>
          </details>
        </div>
