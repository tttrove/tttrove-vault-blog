...
const sortStorageKey = "explorerSort";
...
function createSortFn(sortMode, fallbackSortFn) {
  const [field, directionName] = sortMode.split("-");
  const direction = directionName === "desc" ? -1 : 1;

  return (a, b) => {
    if (a.isFolder !== b.isFolder) return a.isFolder ? -1 : 1;

    if (field === "name" || a.isFolder) {
      return compareNames(a, b, field === "name" ? direction : 1);
    }

    const aDate = Date.parse(a.data?.[field] || "");
    const bDate = Date.parse(b.data?.[field] || "");
    const aHasDate = !Number.isNaN(aDate);
    const bHasDate = !Number.isNaN(bDate);

    if (aHasDate && bHasDate && aDate !== bDate) return direction * (aDate - bDate);
...
async function buildFileTrie(dataFns, sortMode) {
  try {
    ...
    return processTrie(trie, createSortFn(sortMode, sortFn), filterFn, mapFn);
  } catch (e) {
...
async function handleNavOrRender(e) {
  ...
  try {
    ...
    for (const explorer of allExplorers) {
      ...
      for (const sortButton of explorer.querySelectorAll("[data-sort]")) {
        const isSelected = sortButton.dataset.sort === sortMode;
        sortButton.setAttribute("aria-checked", isSelected ? "true" : "false");
        sortButton.classList.toggle("selected", isSelected);
      }

      // Build and render the tree
      console.log("[Explorer] Starting tree build...");
      const trie = await buildFileTrie(dataFns, sortMode);
      ...
      const sortButtons = explorer.querySelectorAll("[data-sort]");
      for (const button of sortButtons) {
        const sortClickHandler = function () {
          localStorage.setItem(sortStorageKey, this.dataset.sort);
          this.closest("details")?.removeAttribute("open");
          document.dispatchEvent(new CustomEvent("render", { detail: { url: currentSlug } }));
        };
        button.addEventListener("click", sortClickHandler);
        cleanupHandlers.push(() => button.removeEventListener("click", sortClickHandler));
      }
