...
describe("ContentIndex emitter", () => {
  ...
  it("includes created and modified dates in contentIndex.json", async () => {
    const emitter = ContentIndex();
    const created = new Date("2025-01-02T03:04:05.000Z");
    const modified = new Date("2025-06-07T08:09:10.000Z");
    const content: ProcessedContent[] = [
      createPage({ slug: "dated", text: "hi", created, modified }),
    ];
    await emitter.emit(createCtx(outputDir), content, createResources());

    const index = await readJson<Record<string, { created?: string; modified?: string }>>(
      path.join(outputDir, "static", "contentIndex.json"),
    );
    expect(index["dated"]!.created).toBe(created.toISOString());
    expect(index["dated"]!.modified).toBe(modified.toISOString());
  });
